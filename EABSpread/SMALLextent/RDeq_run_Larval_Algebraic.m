...
%New vectors
  %Parasitoid Vectors
  Poa=(0.62-0.12).*rand(s,1,N*M)+0.12;%Poa=(0.155-0.03).*rand(s,1,N*M)+0.03;%%%%%This should just be a rate not normalized by month. Just a flat reduction in fecundity.
  Pmp=(0.83268-0.02027).*rand(s,1,N*M)+0.02027;%%%%%%%This is slightly trickier. Assume that 1 quarter of growth per month, and the total reduction is 3% to 57%. %Now Assuming Future Value of Annuity to solve values
  Pps=1-(0.02.*randn(s,1,N*M)+0.32);%%%%%%%%%This is easier. It should be fine as just an annual rate. Ill have to check the sources on it. Additionally, any sources on resistance could go here eventually.
