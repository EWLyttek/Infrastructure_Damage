...
for i=proceed
	...
	STdot(i)=(C*EABL(i)*ASH(i));%-(FALL*ST(i))-(RD*ST(i));
	FTdot(i)=0;%(FALL*ST(i))-(RF*FT(i));
	COdot(i)=0;%2013.23890044*((RD*ST(i))+(RF*FT(i)));%This number is derived from the average carbon content of ash trees in NJ (12inch dbh class)
	FCdot(i)=0;
	ASHicVdot(i)=0;
	RISKicVdot(i)=0;
	Rdot(i)=0;
