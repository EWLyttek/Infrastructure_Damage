...
%Make Initial Conditions. Reshape vectors to matrix ~> back to vector. Ensures consistency in length between vectors.
  ...
  %Note to change X and Y values. Reverse Definition.
    sizex=755;
    sizey=929;
  ...
  %Now turn matrix into a long vector
  	[N, M] = size(ASHic);
    %Preallocate Vector Lengths
      ASHicV = zeros(1,sizex*sizey);
      EABLicV = zeros(1,sizex*sizey);
      EABAicV = zeros(1,sizex*sizey);
      FCicV = zeros(1,sizex*sizey);
      RISKicV = zeros(1,sizex*sizey);
      XcoordV = zeros(1,sizex*sizey);
      YcoordV = zeros(1,sizex*sizey);
    %Now turn matrix into a long vector
    	for i = [1:N]
    	    for j = [1:M]
...
finalmat='RESULT.mat';
dlmwrite('Comma_Delimited_Export_Deterministic.txt',FinalStateMat,'precision','%.6f')
save(finalmat)
clearvars
